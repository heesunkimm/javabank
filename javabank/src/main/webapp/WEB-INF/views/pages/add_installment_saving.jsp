...
<jsp:include page="index_top.jsp"/>
    ...
    <section id="add_installment_saving" class="content add_bank">
        <p>정기적금 개설</p>
        <div class="txt_box bg_blue">
            <p>java<em>bank</em></p>
            <p>${loginUser}님의 정기적금</p>
        </div>
        <form name="productForm" action="add_installment_saving" method="post">
        <input class="csrfToken" type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
            ...
            <div>
                ...
                <div>
                    <label>
                        <input type="text" name="monthlyPayment" placeholder="최대 100만원" value="">
                    </label>
                </div>
            </div>
            <div>
                ...
                <div>
                    <select name="interestRate">
                        <option value="0.033">6개월 (기본 3.3%)</option>
                        <option value="0.035">1년 (기본 3.5%)</option>
                    </select>
                </div>
            </div>
            <div>
...
<script>
	$(document).ready(function() {
	    ...
	    $('.addBtn').on("click", function() {
	    	let payment = $("input[name='monthlyPayment']").val();
	    	let accountPw = $("input[name='productPw']").val();
			let accountPw2 = $("input[name='productPw02']").val();
